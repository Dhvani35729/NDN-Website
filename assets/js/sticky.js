if($('.sticky-mobile').length>0){;(function(window,document,undefined){'use strict';function extend(a,b){for(var key in b){if(b.hasOwnProperty(key)){a[key]=b[key];}}
return a;}
...
headsUp.prototype={defaults:{delay:50,sensitivity:20},init:function(selector){var self=this,options=self.options,selector=self.selector,oldScrollY=0,winHeight;function resizeHandler(){winHeight=window.innerHeight;return winHeight;}
...
if(selector){resizeHandler();window.addEventListener('resize',throttle(resizeHandler),false);window.addEventListener('scroll',throttle(scrollHandler,100),false);}}};window.headsUp=headsUp;})(window,document);new headsUp('.sticky-mobile');}